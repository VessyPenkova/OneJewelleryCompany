...
public class InventoryServiceTests
{
    ...
    [TestCase(0)]
    [TestCase(-1)]
    public async Task ValidateCart_rejects_non_positive_quantity(int qty)
    {
        await using var db = TestDb.Create();

        Assert.That(
            await new InventoryService(db).ValidateCartAsync(
                new[]
                {
                    new CartItem
                    {
    ...
    [Test]
    public async Task ValidateCart_rejects_unknown_line()
    {
        await using var db = TestDb.Create();
    ...
    public async Task Decrement_paid_order_decrements_ready_jewel()
    {
        ...
        await new InventoryService(db)
            .DecrementOnPaidOrderAsync(order);
    ...
    [Test]
    public async Task ApplyInvoice_rejects_empty_invoice()
    {
        await using var db = TestDb.Create();
        ...
        Assert.ThrowsAsync<InvalidOperationException>(
            async () =>
                await new InventoryService(db)
                    .ApplyInvoiceAsync(new Invoice()));
    }

    [Test]
    public async Task ApplyInvoice_rejects_line_with_multiple_targets()
    {
        await using var db = TestDb.Create();
    ...
    public async Task AdjustCollectionStock_prevents_negative_stock()
    {
        ...
        await sut.AdjustCollectionStockAsync(
            c.Id,
            -2);
