...
using OneJevelsCompany.Web.Models.Shop;
...
using ShopConfigureComponentVm = OneJevelsCompany.Web.Models.Shop.ConfigureComponentVm;
using CoreDesignOrder = OneJevelsCompany.Core.Entities.DesignOrder;
using CartItem = OneJevelsCompany.Core.Entities.CartItem;
using JewelCategory = OneJevelsCompany.Core.Enums.JewelCategory;
...
namespace OneJevelsCompany.Web.Controllers
{
    public class ShopController : Controller
    {
        ...
        public async Task<IActionResult> Build(
            JewelCategory category = JewelCategory.Necklace)
        {
        ...
        public async Task<IActionResult> Build([FromBody] BuildRequest req)
        {
            ...
            var category = Enum.TryParse<JewelCategory>(
            ...
            var sku = $"CUSTOM-{category}-{Guid.NewGuid():N}".ToUpperInvariant();
        ...
        public async Task<IActionResult> AddReady(int id, int qty = 1)
        {
            ...
            _cart.AddToCart(
                ...
                new CartItem
                {
                    ...
                    ComponentsSummary = string.Join(
                        ", ",
                        ready.Components
                            .Where(c => c.Component != null)
                            .Select(c => c.Component!.Name)),
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            var vm = new ComponentDetailsVm
            {
                ...
                SizeLabel = c.SizeLabel ?? string.Empty,
                Dimensions = c.Dimensions ?? string.Empty,
        ...
        public async Task<IActionResult> Configure(int id)
        {
            ...
            var vm = new ShopConfigureComponentVm
            {
                Id = c.Id,
                Name = c.Name,
                ImageUrl = c.ImageUrl,
            ...
            ShopConfigureComponentVm form)
        {
            ...
            if (max == 0)
            {
                ModelState.AddModelError(
                    "",
                    "This component is out of stock.");
            ...
            var dim =
                string.IsNullOrWhiteSpace(form.SelectedDimension)
                    ? "Std"
                    : form.SelectedDimension.Trim();

            var sku =
                $"COMP-{c.Id}-{dim}".ToUpperInvariant();
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<IActionResult> Design(JewelCategory category = JewelCategory.Necklace)
        {
        ...
        public async Task<IActionResult> Design(DesignPostVm form)
        {
            ...
            using (var doc = JsonDocument.Parse(
                       string.IsNullOrWhiteSpace(form.SegmentsJson)
                           ? "[]"
                           : form.SegmentsJson))
            {
                ...
                {
                    var id =
                        el.GetProperty("componentId").GetInt32();
            ...
            {
                TempData["err"] =
                    "Please add at least one bead to the sequence.";
            ...
            foreach (var s in segments)
            {
                ...
                totals[s.ComponentId] +=
                    s.Count * form.Repeat;
            }

            var comps =
                await _products.GetComponentsAsync();
            ...
            var sku =
                $"DESIGN-{Guid.NewGuid():N}".ToUpperInvariant();
            ...
            _cart.AddToCart(
                ...
                new CartItem
                {
                    Sku = sku,
                    Title =
                        string.IsNullOrWhiteSpace(form.DesignName)
                            ? "Custom Bracelet"
                            : form.DesignName.Trim(),
            ...
            TempData["ok"] =
                "Your custom design was added to the cart.";
        ...
        [ValidateAntiForgeryToken]
        [AllowAnonymous]
        public async Task<IActionResult> SubmitDesign(
            [FromBody] SubmitDesignVm vm)
        {
            if (vm == null ||
                vm.Rows == null ||
                vm.Rows.Count == 0)
            ...
            var serverEstimate =
                vm.Rows
                    ...
                    .Sum(
                        r =>
                            components[r.ComponentId].Price *
                            r.Count);

            var patternJson =
                JsonSerializer.Serialize(vm.Rows);

            var order = new CoreDesignOrder
            {
                Category = cat,
                ...
                BeadMm = beadMm,
                Mode =
                    string.IsNullOrWhiteSpace(vm.Mode)
                        ? "circle"
                        : vm.Mode!,
                Tilt = vm.Tilt ?? 65,
                Rotate = vm.Rotate ?? -10,
        ...
        {
            return View(
                "~/Views/Shop/DesignSubmitted.cshtml",
                id);
        }
        ...
        public async Task<IActionResult> Designs()
        {
            ...
            var customs =
                await _db.DesignOrders
                    .Where(
                        o =>
                            o.PreviewDataUrl != null &&
                            o.PreviewDataUrl != "")
            ...
            var customCards =
                customs
                    .Where(x => x.Last != null)
                    .Select(
                        x => new DesignGalleryItem
                        {
                            Title =
                                $"Custom design #{x.Last!.Id}",
                            DataUrl =
                                x.Last!.PreviewDataUrl,
                            Category =
                                x.Last!.Category.ToString(),
                            Rating = x.Count,
                            LinkUrl =
                                Url.Action(
                                    nameof(DesignSubmitted),
